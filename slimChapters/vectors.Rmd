...
Notice that for the mutation type, we can provide multiple mutation types using the syntax: `c(m1, m2, m3)`. Likewise, we now set their ratio in the third argument, `c(50, 1, 10)`. 

This essentially says that: 

* Genomic element `"g"` can have mutations of type `m1`, `m2`, and `m3`
...
* We have arranged the two types of DNA into a genome, in the order `g1`, `g2`, `g3`
...
Looking at the mutation tracker, we can see different mutation types color coded by the fitness effect - yellow for neutral mutations, green for favorable mutations, and red for deleterious mutations. Note that nin the dark blue region (`g2`), there are only neutral mutations. This agrees with our setup of the genome, as `g2` can only have mutations of type `m1`.
